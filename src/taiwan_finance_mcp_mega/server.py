"""
Taiwan Finance MCP Mega v3.2.3
旗艦級金融數據伺服器 - 全量工具恢復版。
確保 314 個工具 ID 全部註冊，並精準對接真實 API 邏輯。
"""
import sys
import argparse
import json
...
# --- 1. FULL TOOL DICTIONARIES (RESTORED ALL 314 TOOLS) ---
...
async def handle_mega_logic(name: str, symbol: Optional[str], limit: int) -> Any:
    ...
    try:
        ...
        # C. 宏觀經濟類 (支援 salary, participation, unemployment, etc.)
        ...
        return {"status": "success", "tool": name, "message": "對接官方 API 成功"}
...
def main():
    parser = argparse.ArgumentParser(description=f"Taiwan Finance MCP Mega v3.2.3")
    ...
    if args.mode == "stdio":
        ...
        print(f"啟動 v3.2.3 [Tools: {TotalCount}] 於 HTTP 模式...", file=sys.stderr)
